...
public class Tcrypt {
    ...
    public static void main(String[] args) {
        ...
        if (1 == 1){ // for gui branch, am too lazy to do a proper second branch
    ...
    public static void encryptFile(String filepath, MyFrame myFrame) {
        try {
            ...
            byte[] passwordHash = MessageDigest.getInstance(ALGORITHM)
            ...
            while ((bytesRead = input.read(fileBuffer)) != -1) {
                ...
                try {
                    ...
                    Log.log("Error at setting percent bar", Level.WARNING);
                }
            ...
            Log.log("Time to Encrypt: " + (((end - start) - (intermediateTime2 - intermediateTime1)) / 1_000_000_000.0)
                    + "s", Level.INFO);
        } catch (IOException e) {
    ...
    public static void decryptFile(String filePath, String keyPath, MyFrame myFrame) {
        try {
            ...
            String decryptedPath = filePath.replace(decryptedPathElements[decryptedPathElements.length - 1],
                    originalFileName);
            ...
            try (FileInputStream keyInput = new FileInputStream(keyPath);
                ...
                while (remaining > 0) {
                    ...
                    for (int i = 0; i < fileRead; i++) {
                        ...
                        decryptedBuffer[i] = (byte) (fileBuffer[i] ^ keyBuffer[i]);
                    }
            ...
            Log.log("Decrypted Format Version: " + TcryptFormatVersion + " Tcrypt Version: " + TcryptVersion,
                    Level.INFO);
            Log.log("Time to Decrypt: " + (((end - start) - (intermediateTime2 - intermediateTime1)) / 1_000_000_000.0)
                    + "s", Level.INFO); // Intermediate time --> so slow user
    ...
    public static void verifyFile(String filepath, String encryptedFilePath) {
        try {
            ...
            String storedHash = new String(
                    Arrays.copyOfRange(encryptedBytes, encryptedBytes.length - 64, encryptedBytes.length),
